...
const Collectibiles = () => {
  ...
  useEffect(() => {
    ...
    const fetchERC20Tokens = async (walletaddress) => {
        const nfttokens = await axios.get(`https://deep-index.moralis.io/api/v2/${walletaddress}/nft`, { params: { chain: 'eth', format: 'decimal' }, headers: { accept: 'application/json', 'X-API-Key': 'i9pF2GEg0P3w0pYZ4xzNDkZq4sXnxHidd7fXtO3ETfNNIm1t8Aq4LAUcAv54wk4z' } })
        let filteredNftTokens = nfttokens.data.result.filter(item => item.symbol != 'ENS' && item.token_address != '0x495f947276749ce646f68ac8c248420045cb7b5e' && item.token_address != '0x7c770595a2be9a87cf49b35ea9bc534f1a59552d');
